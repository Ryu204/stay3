...
void check_transitive_update(tree_context &ctx, const entities &es) {
    ...
    {
        auto depth2_tf = ctx.ecs().get<transform>(es.depth2);
        depth2_tf->scale(2.F);
    }
    sync_global_transform(ctx);
    auto depth3_tf = ctx.ecs().get<const global_transform>(es.depth31);
...
void check_parent_entity_added_update(tree_context &ctx, entities es) {
    ...
    sync_global_transform(ctx);
    {
        ...
        auto depth2_tf = ctx.ecs().emplace<transform>(es.depth2);
        depth2_tf->scale(5.F);
    }
    sync_global_transform(ctx);
