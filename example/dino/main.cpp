...
//     static void update_anim_frame(ecs_registry &reg, entity en, const animation &anim, animation_status &anim_status) {
//         anim_status.index %= anim.frames.size();

//         const auto &texture = *rmesh.material.get(reg)->texture.get(reg);
//         auto sprite_builder = reg.get<mut<mesh_sprite_builder>>(en);
//         sprite_builder->texture = &texture;
//         sprite_builder->texture_rect = anim.frames[anim_status.index];
//     }
...
//         make_hard_dependency<mesh_sprite_builder, animation>(ctx.ecs(), mesh_sprite_builder{.pixels_per_unit = data::pixels_per_unit});
//     }
